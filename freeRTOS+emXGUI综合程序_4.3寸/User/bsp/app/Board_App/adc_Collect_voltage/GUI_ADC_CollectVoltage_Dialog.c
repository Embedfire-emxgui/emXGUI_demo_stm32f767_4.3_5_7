...
void Circle_Paint(HWND hwnd, HDC hdc)
{
  ...
  BitBlt(hdc, CircleSize/2-162/2, CircleSize/2-162/2, 162, 162, hdc_adc_png[hdc_adc_circle], 0, 0, SRCCOPY);     // 270 是图片的大小
...
static LRESULT	CollectVoltage_proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  switch(msg)
  {
    case WM_CREATE:
    {
      ...
      // 创建" ADC 采集窗口"的控件.
      CreateWindowEx(WS_EX_NOFOCUS, &wcex,L"---",WS_CLIPCHILDREN|WS_VISIBLE,rc.x,rc.y,rc.w,rc.h,hwnd,ID_ADV_WIN,NULL,NULL);
            
      CreateWindow(BUTTON, L"O", WS_TRANSPARENT|BS_FLAT | BS_NOTIFY |WS_OWNERDRAW|WS_VISIBLE,
                  444, 0, 36, 36, hwnd, eID_ADC_EXIT, NULL, NULL); 
