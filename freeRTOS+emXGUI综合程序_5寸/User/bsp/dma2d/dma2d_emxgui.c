...
void DMA2D_IRQHandler(void)
{
  ...
  ulReturn = taskENTER_CRITICAL_FROM_ISR(); 
	...
	DMA2D->IFCR |= DMA2D_IFCR_CTCIF; //clr isr;
	NVIC_ClearPendingIRQ(DMA2D_IRQn);

	dma2d_rdy=TRUE;
	GUI_SemPostISR(sem_rdy);
//  xSemaphoreGiveFromISR((SemaphoreHandle_t)sem_rdy,NULL);
  /* 退出临界段 */
...
BOOL DMA2D_FillRect(const SURFACE *pSurf,int x,int y,U16 w,U16 h,U32 color)
{
	...
	case SURF_RGB565:
		...
		break;
	case SURF_ARGB4444:
		DMA2D->OPFCCR = DMA2D_OUTPUT_ARGB4444;
		break;
	case SURF_XRGB8888:
    ...
    DMA2D_Execu();
    DMA2D_WaitDone();
    SCB_InvalidateDCache_by_Addr((uint32_t*)DMA2D->OMAR,pSurf->WidthBytes*h);
	return TRUE;
...
BOOL DMA2D_DrawBitmap_ARGB(const SURFACE *pSurf,int x,int y,U16 w,U16 h,int width_bytes,const U8 *bits,U32 color_format)
{
    ...
    switch(pSurf->Format)
    {
      ...
      case SURF_ARGB4444:
        DMA2D->OPFCCR = DMA2D_OUTPUT_ARGB4444;
        DMA2D->BGPFCCR =DMA2D_INPUT_ARGB4444;
        break;
      case SURF_XRGB8888:
        DMA2D->OPFCCR = DMA2D_OUTPUT_RGB888;
        DMA2D->BGPFCCR =DMA2D_INPUT_RGB888;
        break;
      case SURF_ARGB8888:
        DMA2D->OPFCCR = DMA2D_OUTPUT_ARGB8888;
    ...
    DMA2D->FGPFCCR = DMA2D_NO_MODIF_ALPHA|color_format;
    DMA2D->FGMAR = (U32)bits;
   // DMA2D->FGOR   = (width_bytes>>1)-w;
...
*/
		...
		DMA2D_DrawBitmap_ARGB(pSurf,20+10,30+10,128,64,128*4,(const U8*)buf32,DMA2D_INPUT_ARGB8888);
		SYS_msleep(1000);
		GUI_memset32(buf32,ARGB8888(80,255,255,0),128*64);
		DMA2D_DrawBitmap_ARGB(pSurf,20+20,30+20,128,64,128*4,(const U8*)buf32,DMA2D_INPUT_ARGB8888);
		SYS_msleep(1000);
