...
//    GUI_GET_DATA  get_data;
    ...
    res = BMP_DrawEx(hdc,x,y,&get_data,lprc);
		...
		/* 全部读取的方式打开 */
		int rw=0;
		uint32_t bmp_size = f_size(file);
		void * tempbuf = NULL;
		tempbuf = GUI_VMEM_Alloc(bmp_size);
		f_read(file, tempbuf, (UINT)bmp_size, (UINT *)&rw);
		res = BMP_Draw(hdc,0,0,tempbuf,lprc);
		GUI_VMEM_Free(tempbuf);
#endif
    /* 关闭文件 */
    f_close(file);
